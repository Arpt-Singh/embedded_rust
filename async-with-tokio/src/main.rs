   ...
   // test_something().await;
    ...
    loop {
        if handle01.is_finished() && handle02.is_finished() {
            println!("all racers has finished!");
            break;
        }
...
impl F1Racer {
    fn new() -> F1Racer {
        return F1Racer {name:"Max Verstappen".to_string(), laps:5, completed_laps:0,best_lap_time:255, lap_times:vec![87u8,64, 126, 95, 76]};
    }
    fn do_loop(&mut self) {
